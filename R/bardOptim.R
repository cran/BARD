...
# refineGreedyPlan
#
# This uses hill-climbing to refine plans. It will always accept the best
...
refineGreedyPlan <- function(plan, score.fun, displaycount=NULL, historysize=0, dynamicscoring=FALSE,   tracelevel=1 ) {
  ...
  candpairs<-function(i) {
     as.matrix(expand.grid(i,neighbors(nb,i)))
  }
   ...
   greedyScore <-
        ...
        boundCheck=TRUE, tracelevel=max(0,tracelevel-1))
  ...
  repeat {
     ...
     if (tracelevel) {
        print(paste("MAJOR ITERATION ",mitercount,": Checking",dim(lpairs)[1],"candidates",Sys.time())) 
        flush.console()
     }   
     if (dynamicscoring) {
     ...
     if (sum(lpairsScores[bestswitch])>=sum(prevscore)) {
      break
      # LOCAL OPTIMUM
     }
     ...
     if (tracelevel) {
        print(paste("MAJOR ITERATION: BEST Score",score,"iteration",mitercount,Sys.time())) 
        flush.console()
     }           
