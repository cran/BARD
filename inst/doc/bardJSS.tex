...
\Abstract{
\pkg{BARD} is the first (and currently only) open source software package for redistricting and redistricting analysis. \pkg{BARD} provides methods to create, display, compare, edit, automatically refine, evaluate, and profile political districting plans.  \pkg{BARD} aims to provide a fraimwork for  scientific analysis of  redistricting plans and as a step toward facilitating wider participation in the creation of new plans
...
an exact optimization solution, \pkg{BARD} makes use of a variety of selectable metaheuristics, including genetic algorithms, GRASP, and simulated annealing, that can be use to refine existing or randomly-generated redistricting plans based on user-determined criteria. 

Furthermore, \pkg{BARD} supports the ability to randomly generate redistricting plans, and to generate profiles of plans for different scoring weights. 
This functionality  can be used both to explore trade-offs among criteria and to make inferences regarding the intent behind existing redistricting plans. 

Because of the computational intensity of these methods,  performance 
is an important criterion in the design and implementation of \pkg{BARD}. The program implements performance enhancements such as evaluation caching, explicit memory management, and distributed computing across \pkg{snow} clusters.
}
...
Unfortunately, early computerized redistricting systems were prohibitively expensive to all but state governments or political parties, effectively shutting out the public from the redistricting process. Computer hardware and specialized redistricting GIS software have since become more affordable. However, specialized software packages for redistricting still remain unusually expensive -- costing thousands of dollars. We aim to make the redistricting process more open, by  offering the first open-source redistricting software. We call our computer program \pkg{BARD} for \textbf{B}etter \textbf{A}utomated 
...
First, \pkg{BARD} can read and process redistricting data. \pkg{BARD} can read and write files representing redistricting plans in the standard ESRI ``shapefile'' format so that you can use the output of \pkg{BARD} in other GIS packages, or read existing plans into \pkg{BARD}. (Also, at this stage \pkg{BARD}  will generate contiguity analysis of maps in preparation for later analysis.) In this stage the user selects the geography to load along with any demographic and political variables. The program also reads in existing plans, if any. \footnote{We intend for \pkg{BARD} to be used as a complement to a general GIS package, specifically to optimize and evaluate districts: No freely available GIS package has either capability. And \pkg{BARD} does not have the ability to manipulate map data as a general GIS package would. However, \pkg{BARD} does support some basic interactive operation, and readily accepts the most common GIS data format. One can use \pkg{BARD} to load redistricting plans and display them.}


Second, \pkg{BARD} can be used to modify and evaluate redistricting plans. \pkg{BARD} will generate textual and graphical reports on a single plan or comparing multiple plans. Currently \pkg{BARD} will show the precinct-level differences between pairs of plans, count `holes' in plans, compute common compactness scores, calculate overall population deviation, and check for contiguity. You can also use \pkg{BARD} to compute many common political measures, such as number of majority-minority districts, plan competitiveness, etc. Since \pkg{BARD} is built on \proglang{R}, it is easy to add any scoring method that you can think of -- and these will automatically be integrated into \pkg{BARD}'s reports.

Third, \pkg{BARD}  can be used for  the \emph{generation} and \emph{refinement} of plans. Plans are generated for use as starting points in the refinement or evaluation process.  The user may also choose to start from pre-existing 
...
functions in \pkg{BARD}. These functions permit users to load existing plans into \pkg{BARD} or instruct 
...
This loads the \pkg{BARD} package:
...
This reads in a standard ``shapefile'' into \pkg{BARD} which will be used as the base map.
...
\newpage
In addition to plan generation, \pkg{BARD}  supports basic interactive editing of plans (by reassigning blocks among districts).
...
Furthermore, \pkg{BARD} can generate reports for plans based on built-in or user-supplied redistricting criteria (as described in the next section). Here is another brief example:
...
2    kmeans          2         2  0.0000000    NA 0.37793995 0.6615332
3    kmeans          3         3  0.0000000    NA 0.07392494 0.5432474
4    kmeans          4         4  0.0000000    NA 0.13276734 0.6475287
5    kmeans          5         5  0.0000000    NA 0.15240584 0.5042317
6    kmeans      Total     Total  0.0000000     0 0.82448922 2.7449241
7  random 1          1         5  0.9629630    NA 0.61087731 0.9210047
8  random 1          2         4  0.9666667    NA 0.58356679 0.9260310
9  random 1          3         2  0.9687500    NA 0.59046596 0.9690781
10 random 1          4         1  0.9655172    NA 0.55380317 0.8999835
11 random 1          5         3  0.9687500    NA 0.59091245 0.9262929
12 random 1      Total     Total  4.8326469     0 2.92962568 4.6423902
...
1           1      5                   93               66       56    18.10
2           2      4                   21               15       47     8.82
3           3      2                  103               78       49    16.40
4           4      1                   39               31       47     9.30
5           5      3                   64               54       45     9.17
...
differences in district level scores, and even the diffences in census unit assignments between districts.

And the differences between districts can be plotted, using a basic plot function. This shows areas in which the two plans overlap:
...
few are used in practice (see \citet{Altman98}, \citet{Niemi91}). \pkg{BARD} provides three functions that calculate the most common geographic compactness scores which capture the dispersion and dissection of districts (how much they are stretched or have chunks removed from them). \pkg{BARD} also provides a parameterized and weighted `moment-of-inertia' score that can be used to measure geographic compactness, population compactness, and cumulative distance from fixed locations, such as candidates homes, warehouses, or schools. Other scores can easily be created in \proglang{R}.
...
All of these criteria are easily calculated in \pkg{BARD}.

In addition, the \pkg{BARD} score 
framework enables any function of model in \pkg{R} to be used in the evaluation of 
...
This simple demonstration shows how \pkg{BARD} can be used to repeatedly generate and refine plans.This shows the results of applying one optimization algorithm to a set of 21 plans, the plan generated by kmeans, plus 20 plans generated by \code{createRandomPlan}. (Since this is computationally intensive, \pkg{BARD} supports distributing this process across a computing cluster.) Each plan is then refined using the score function and optimization function indicated:
...
constitutionally viable plans. \footnote{To keep the run-time of this demonstration manageable, we implement the essential profiling steps  using a quick but primitive optimization method, \code{refineNelderPlan}.  In a 
real (and long running) application we would substitute \code{refineTabuPlan}, \code{refineAnnealPlan}, \code{refineGRASPplan}, or \code{refineGenoudPlan} for \code{refineNelderPlan} above.}  
...
\textit{entire distribution} of plans, whereas \pkg{BARD} requires only that the redistricting authority was reasonable aware of their choices.
...
We use the \proglang{maptools} \citep{Lewin07} data structures for storing 
...
\pkg{BARD} requires the adjacency graph. A standard distance matrix required excessive amounts of memory, 
...
Moreover, each of the built-in score functions provided in \pkg{BARD} is designed to permit dynamic updating.  Each will accept the 
...
back to the score function. When creating weighted scores by hand that 
...
And, to minimize the housekeeping of creating composite dynamic score functions, \pkg{BARD} provides a wrapper function, \code{combineDynamicScores}. 
...
Finally, in order to support scaling out to larger problems, \pkg{BARD} supports the use of clusters of distributed 
...
\pkg{BARD} has now recently been released to the public for the first time.  While \pkg{BARD} offers 
...
\pkg{BARD} is the first and (currently) only open-source system for redistricting. While 
...
presented analyses based on automated redistricting techniques, no tools  that support such methods have previously been 
 made publicly available.  \pkg{BARD} provides tools 
...
As noted above, additional care in the implementation of \pkg{BARD} was needed to avoid making copies of large objects. A particular issue was the assignment of  large objects as attributes of other objects. This occured when redistricting plans contained a reference to the underlying map object on which they which they were based.  We 
developed the idiom of wrapping these objects in an \code{environment} and 
assigning this environment as an attribute, which proved successful at avoiding 
duplication of these large data structure.  We also developed a special-purpose ``shallow'' concatenate operator, which we used to concatenate multiple lists of large objects. This avoided the memory duplication which would normally occur when using the \code{c()} operator.
